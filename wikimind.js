...
function renderAuthForm() {
  ...
  $("authFooter").innerHTML = L
    ...
    : Sg
      ? `<span class="auth-link" onclick="setAuthMode('login')">이미 계정이 있으신가요? 로그인</span>`
...
async function handleEmailAuth(isSignup) {
  ...
  try {
    ...
    showAuthMsg(fbErrMsg(e.code), "err");
  }
...
async function handleReset() {
  const email = $("authEmail").value.trim();
  if (!email) {
    showAuthMsg("이메일을 입력해주세요.", "err");
    return;
  }
  try {
    await sendPasswordResetEmail(auth, email);
    showAuthMsg("재설정 링크를 이메일로 보냈습니다.", "ok");
  } catch (e) {
    showAuthMsg(fbErrMsg(e.code), "err");
  }
}
...
function showAuthMsg(m, type) {
  const e = $("authError");
  e.textContent = m;
  e.style.display = "block";
  e.className = "auth-msg " + type;
}
...
window.handleVerifySignOut = async function () {
  clearInterval(verifyPoller);
  await signOut(auth);
};
...
function startSync(uid) {
  ...
  state.calv4Unsub = onSnapshot(
    ...
    (snap) => {
      if (snap.exists()) {
        try {
          state.nativeEvents = JSON.parse(snap.data().data || "[]");
        } catch (e) {
          state.nativeEvents = [];
        }
      } else {
...
async function pushDoc(docData) {
  ...
  await setDoc(
    doc(db, "users", state.currentUser.uid, "docs", String(docData.id)),
    {
      title: docData.title,
...
async function removeDoc(docId) {
  if (!state.currentUser) return;
  await deleteDoc(
    doc(db, "users", state.currentUser.uid, "docs", String(docId)),
  );
}
...
function renderTree() {
  const tree = $("docTree");
  const docs = state.activeTag
    ? state.docs.filter((d) => d.tags.includes(state.activeTag))
    : state.docs;
  const grouped = {},
    noFolder = [];
  docs.forEach((d) => {
  ...
  tree.innerHTML =
    html ||
    '<div style="padding:12px;font-size:12px;color:var(--text3);text-align:center">문서 없음</div>';
}
...
function renderDocTags(d) {
  ...
  wrap.innerHTML =
    d.tags
      ...
      .join("") +
    `<input type="text" class="tag-add-input" id="tagAddInput" placeholder="태그 추가..." maxlength="20" onkeydown="addTagOnEnter(event)">`;
}
...
function parseMarkdown(md) {
  ...
  md = md
    ...
    .replace(
      /^- \[x\] (.+)$/gm,
      '<li style="list-style:none">✅ <del>$1</del></li>',
    )
    ...
    .replace(
      /^📅\[([a-zA-Z0-9]+)\]\s+(\d{4}-\d{2}-\d{2})(?:\s+(\d{2}:\d{2}))?(?:\s+~(?:\s+(\d{4}-\d{2}-\d{2}))?(?:\s+(\d{2}:\d{2}))?)?\s+([^\n]+)/gm,
      (_, id, sd, st, ed, et, title) => {
        let timeStr = "";
        if (st) timeStr += st;
...
const SLASH_COMMANDS = [
  ...
  },
  {
    ...
    label: "파일 첨부",
    desc: "파일 첨부",
    action: () => triggerFileUpload("file"),
  },
  {
    icon: "📅",
    label: "일정 추가",
    desc: "캘린더 연동 일정",
    action: () => insertScheduleLine(),
  },
];
...
function handleSlashInput() {
  ...
  const filtered = SLASH_COMMANDS.filter(
    (c) => query === "" || c.label.toLowerCase().includes(query) || c.desc.toLowerCase().includes(query)
  );
...
function moveSlash(dir) {
  ...
  state.slashIdx = Math.max(
    0,
    Math.min(items.length - 1, state.slashIdx + dir),
  );
  items.forEach((el, i) =>
    el.classList.toggle("selected", i === state.slashIdx),
  );
}
...
window.selectSlashItem = function (idx) {
  executeSlash(SLASH_COMMANDS[idx]);
};
        ...
        .map((d) => {
          ...
          const prev =
            idx >= 0
              ? "..." +
                d.content
                  .slice(Math.max(0, idx - 20), idx + 50)
                  .replace(/[#*`]/g, "") +
                "..."
      ...
      evs.length > 0
        ? `<div class="mc-dots">` +
          evs
            ...
            .map(
              (ev) =>
                `<div class="mc-dot ${!ev.isDocEvent ? "native" : ""}"></div>`,
            )
            .join("") +
          `</div>`
        : "";
